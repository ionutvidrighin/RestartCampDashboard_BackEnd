...
const getStudentDataByStudentName = async (req, res) => {
  ...
  if (accessKey === appAccessKey) {
    try {
      // query DB for student (by its name) in the collections where he might be:
...
const replaceStudentEmailAddress = async (req, res) => {
  ...
  /**
   * "modificationObject" is an Array of Objects (and one String at the beginning), with form: 
  ...
  **/
  ...
  if (accessKey === appAccessKey) {
    ...
    modificationObject.forEach(dataSet => {
      ...
      // and if does, go through each of its values (refIDs) and call the Update document based on that refID 
