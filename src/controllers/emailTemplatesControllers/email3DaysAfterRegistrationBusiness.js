  ...
  jwt.verify(
    ...
    async (err, data) => {
      if (err) {
        ...
        try {
          const emailSubject = emailSubjectJSON.emailSubjectCompany
      ...
      if (err) {
        ...
        try {
          ...
          Object.assign(emailSubjectJSON, {
            emailSubjectCompany: newEmailSubject
          })
  
          fs.writeFileSync(localJSONpath, JSON.stringify(emailSubjectJSON))
          
          res.status(200).json({value: emailSubjectJSON.emailSubjectCompany, message: 'Subiect E-mail actualizat cu success!'})
        } catch(err) {
          res.status(500).json({message: err})
        }
      }
