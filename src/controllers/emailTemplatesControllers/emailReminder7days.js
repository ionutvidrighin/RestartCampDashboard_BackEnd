...
const fs = require('fs');
const emailSubjectJSON = require('../../Nodemailer/EmailReminder7days/emailSubject.json');
const sendTestEmail = require('../../Nodemailer/EmailReminder7days/sendTestEmailReminder7days');
  ...
  jwt.verify(
    ...
    async (err, data) => {
      if (err) {
        ...
        try {
          const localJSONpath = path.join(__dirname, '../../Nodemailer/EmailReminder7days/emailSubject.json')
  ...
  jwt.verify(
    ...
    async (err, data) => {
      ...
      if (err) {
        ...
        uploadPath = path.join(__dirname, '../../', 'Nodemailer', 'EmailReminder7days/template', sampleFile.name)
  ...
  jwt.verify(
    ...
    async (err, data) => {
      if (err) {
        ...
        const file = path.join(__dirname, '../../', 'Nodemailer/EmailReminder7days/template', 'emailReminder7days.handlebars')
        res.download(file)
      }
    }
  )
  ...
  jwt.verify(
    ...
    async (err, data) => {
      if (err) {
        ...
        const filePath = path.join(__dirname, "../../", "Nodemailer/EmailReminder7days/template", "emailReminder7days.handlebars")
